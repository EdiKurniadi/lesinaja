    ...
    <section className="border-b border-black">
          ...
          {CATEGORIES.map((category) => {
            ...
            return (
              <article key={category} className="border-b border-black p-5 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0 sm:p-7">
                ...
                <div className="mt-8 h-3 border border-black"><div className="h-full bg-black" style={{ width: `${accuracy}%` }} /></div>
      ...
      <div className="p-5 sm:p-8 lg:p-10">
        ...
        {state.attempts.length === 0 ? (
          <div className="mt-7 border border-black bg-white p-8 text-center"><p className="text-xl font-bold">Belum ada hasil try out.</p><p className="mt-2">Selesaikan satu paket untuk melihat perkembanganmu.</p></div>
        ) : (
          <div className="mt-7 overflow-x-auto border border-black bg-white">
            <table className="w-full min-w-[680px] border-collapse text-left">
              <thead><tr className="bg-black text-white"><th className="p-4">Paket</th><th className="p-4">Tanggal</th>{CATEGORIES.map((category) => <th key={category} className="p-4">{category}</th>)}<th className="p-4">Total</th><th className="p-4">Status</th></tr></thead>
              <tbody>{state.attempts.map((attempt) => <tr key={attempt.id} className="border-t border-black"><td className="p-4 font-bold">{getPackage(attempt.packageId)?.title ?? attempt.packageId}</td><td className="p-4 text-sm">{dateLabel(attempt.completedAt)}</td>{CATEGORIES.map((category) => <td key={category} className="p-4"><span className={attempt.scores[category].score >= EXAM_RULES.passingScores[category] ? "font-bold" : "text-destructive"}>{attempt.scores[category].score}</span></td>)}<td className="p-4 font-black">{attempt.totalScore}</td><td className="p-4"><span className={`px-2 py-1 text-xs font-bold ${attempt.passed ? "bg-signal" : "border border-black"}`}>{attempt.passed ? "MEMENUHI" : "BELUM"}</span></td></tr>)}</tbody>
            </table>
          </div>
        )}
      </div>
